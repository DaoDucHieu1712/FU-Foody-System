...
const ViewLikePost = ({ likeNumber, likedBy }) => {
  ...
  useEffect(() => {
    setDisplayLikedBy(
      likedBy
        ? likedBy.filter((user) => user.isLike === true).slice(0, 5)
        : []
    );
  }, [likedBy]);
    ...
    <>
      ...
      >
        ...
        {isHovered && (
          <div className="absolute w-max max-h-40 bg-white p-3 rounded shadow-md mt-1">
            <ul>
              {displayLikedBy.map((user) => (
                <li key={user.userId}>{user.username}</li>
              ))}
              {likedBy &&
                likedBy.filter((user) => user.isLike === true).length > 5 && (
                  <li>
                    và{" "}
                    {likedBy.filter((user) => user.isLike === true).length - 5}{" "}
                    người khác ...
          ...
          {displayLikedBy.map((user) => (
            ...
            >
              ...
              <Typography
                ...
                className="py-1 px-2 rounded bg-primary text-white cursor-pointer"
              >
                Xem thông tin
              </Typography>
            </li>
