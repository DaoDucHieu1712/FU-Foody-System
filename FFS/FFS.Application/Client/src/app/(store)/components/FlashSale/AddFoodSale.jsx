...
const AddFoodSale = ({ getFoodList, dateS, dateE }) => {
	...
	const getSelectedItem = (itemId) => {
		...
		return {
			...
			categoryName: inventory.find((item) => item.id === numericItemId)
				?.categoryName,
		};
	};
	...
	const handleCheckAll = () => {
		...
		const updatedSelectedItems = newCheckedState
			? [...selectedItems, ...inventory.map(({ id }) => getSelectedItem(id))]
	...
	const handleCheckSingle = (itemId) => {
		...
		const newCheckedItems = {
			...checkedItems,
			[numericItemId]: !checkedItems[numericItemId],
		};
		const newAllCheckedItems = {
			...allCheckedItems,
			[numericItemId]: !allCheckedItems[numericItemId],
		};
	...
	return (
		<>
			<Button
				color="white"
				className="flex mt-2 gap-2 text-orange-500 border-solid border-2 border-orange-500"
				onClick={handleOpen}
			>
				<svg
			...
			>
				...
				>
					...
					<CardBody className="p-0 mt-1">
						<table className="w-full min-w-max table-auto">
							...
							<tbody>
								{inventory.map(
									(
										{ id, imageURL, foodName, quantity, categoryName },
										index
									) => {
					...
					<div className="flex px-3 pb-2 gap-2 justify-end w-full">
						...
						<Button className="bg-primary right-0" onClick={handleSubmit}>
							Xác nhận
						</Button>
					</div>
				</Card>
